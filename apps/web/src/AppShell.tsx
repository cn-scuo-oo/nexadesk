...
function DesktopPet({
  ...
  return (
    <div className="desktop-pet-window">
      ...
      <strong>{variant ?? "nexabot"}</strong>
      {mood ? <small>{mood}</small> : null}
      {taskTitle ? <small>{taskTitle}</small> : null}
    </div>
  );
}
...
type TaskCardProps = {
  task: { id?: string; name?: string; title?: string; status?: string };
  agents: AgentProfile[];
};
...
function TaskCard({ task, agents }: TaskCardProps) {
  return (
    <div className="task-card">
      ...
      {agents.length ? <small>{agents.length} agents available</small> : null}
    </div>
  );
}
...
function ApprovalCard({
  ...
  return (
    <div className="approval-card">
      ...
      <div>
        <button onClick={() => onResolve(approval, true)} type="button">
          批准
        </button>
        <button onClick={() => onResolve(approval, false)} type="button">
          拒绝
        </button>
      </div>
    </div>
...
function ApprovalHistoryCard({ history, agent }: { history: ApprovalHistoryEntry; agent?: AgentProfile | undefined }) {
  return (
    <div className="approval-history-card">
      <strong>{history.action}</strong>
      <span>{history.decision}</span>
      {agent ? <small>{agent.name}</small> : null}
    </div>
  );
}
...
function WorkspaceFilePanel({
  ...
  const entries = result?.entries ?? fallbackFiles.map((file) => ({
  ...
  return (
    <div className="workspace-file-panel">
      ...
      <div className="workspace-file-panel-actions">
        ...
        <button disabled={sending} onClick={() => onAskAgent(currentPath)} type="button">
          ?????
        </button>
        <button onClick={onClearRecentFiles} type="button">
          ????
        </button>
      </div>
      ...
      <div className="workspace-file-panel-footer">
        <button onClick={() => onOpenPath(".")} type="button">
          ?????
        </button>
        <span>{result?.entries.length ?? fallbackFiles.length} items</span>
      </div>
    </div>
  );
}
...
function MessageBubble({
  ...
  return (
    <div className={`message-bubble ${message.role}`}>
      ...
      <p>{message.content}</p>
      {compactTools ? <small>Compact tools</small> : null}
      {agents?.length ? <small>{agents.length} agents</small> : null}
    </div>
  );
}
...
function renderProviderNote(
  ...
  const parts = [draft.name];
  if (savedId && savedId === draft.id) {
    parts.push("已保存");
  }
  if (test?.checkedAt) {
    parts.push(`测试于 ${formatProviderCheckTime(test.checkedAt)}`);
  }
  if (refresh?.checkedAt) {
    parts.push(`模型刷新于 ${formatProviderCheckTime(refresh.checkedAt)}`);
  }
  return parts.join(" · ");
}
