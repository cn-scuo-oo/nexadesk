...
import type { AppSettings, MemoryEntry, SessionSummary, RuntimeTelemetryEntry } from "@nexadesk/shared";
...
export function getMemoryEntriesByKind(kind: string): MemoryEntry[] {
  const rows = getDb().prepare("SELECT * FROM memory_entries WHERE kind = ? ORDER BY pinned DESC, updated_at DESC").all(kind) as any[];
  return rows.map(rowToMemoryEntry);
}

export function searchMemoryEntries(query: string): MemoryEntry[] {
  const pattern = `%${query}%`;
  const rows = getDb().prepare("SELECT * FROM memory_entries WHERE title LIKE ? OR content LIKE ? OR tags LIKE ? ORDER BY pinned DESC, updated_at DESC").all(pattern, pattern, pattern) as any[];
  return rows.map(rowToMemoryEntry);
}
...
export function insertMemoryEntry(entry: MemoryEntry): void {
  ...
  ).run(entry.id, entry.kind, entry.title, entry.content, JSON.stringify(entry.tags), entry.source ?? null, entry.pinned ? 1 : 0, entry.createdAt, entry.updatedAt);
}
...
export function updateMemoryEntry(id: string, patch: Partial<MemoryEntry>): void {
  ...
  ).run(patch.title ?? existing.title, patch.content ?? existing.content, JSON.stringify(patch.tags ?? JSON.parse(existing.tags)), patch.pinned !== undefined ? (patch.pinned ? 1 : 0) : existing.pinned, id);
}
...
export function insertSessionSummary(summary: SessionSummary): void {
  ...
  ).run(summary.id, summary.sessionId, summary.title, summary.summary, summary.agentId ?? null, summary.createdAt, summary.durationMs ?? null, summary.messageCount);
}
...
export function insertTelemetry(entry: RuntimeTelemetryEntry): void {
  ...
  ).run(entry.id, entry.sessionId, entry.providerName, entry.model, entry.startedAt, entry.completedAt ?? null, entry.firstTokenMs ?? null, entry.durationMs ?? null, entry.inputTokens, entry.outputTokens, entry.totalTokens, entry.status, entry.error ?? null, entry.messagePreview ?? null);
}

/* ── MCP Tool Policies ── */
export function getAllToolPolicies(): Array<{ toolId: string; serverId: string; permission: string; note?: string }> {
  return getDb().prepare("SELECT tool_id as toolId, server_id as serverId, permission, note FROM mcp_tool_policies").all() as any[];
}
...
/* ── Agent Teams ── */
export function getAllTeams(): Array<{ id: string; name: string; emoji: string; description: string; agentIds: string[]; workflow: string }> {
  const rows = getDb().prepare("SELECT * FROM agent_teams ORDER BY created_at DESC").all() as any[];
  return rows.map((r) => ({ id: r.id, name: r.name, emoji: r.emoji, description: r.description, agentIds: JSON.parse(r.agent_ids), workflow: r.workflow }));
}

export function insertTeam(team: { id: string; name: string; emoji: string; description: string; agentIds: string[]; workflow: string }): void {
  getDb().prepare(
...
export function logAudit(action: string, entity: string, entityId?: string, detail?: string): void {
  getDb().prepare("INSERT INTO audit_log (action, entity, entity_id, detail) VALUES (?, ?, ?, ?)").run(action, entity, entityId ?? null, detail ?? null);
}

export function getAuditLog(limit = 50): Array<{ id: number; action: string; entity: string; entityId?: string; detail?: string; createdAt: string }> {
  const rows = getDb().prepare("SELECT * FROM audit_log ORDER BY created_at DESC LIMIT ?").all(limit) as any[];
  return rows.map((r) => ({ id: r.id, action: r.action, entity: r.entity, entityId: r.entity_id ?? undefined, detail: r.detail ?? undefined, createdAt: r.created_at }));
}
